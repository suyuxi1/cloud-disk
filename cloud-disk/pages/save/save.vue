<template>
	<view>
		<view class="bg-light flex align-center">
			<view class="px-2 py-2 text-secondary" hover-class="bg-hover-light" @click="handleBreadcrumb({ id: 0 })">根目录</view>
			<block v-for="(item, index) in dirs" :key="index">
				<view class="py-2">/</view>
				<view class="px-2 py-2 text-secondary" hover-class="bg-hover-light" @click="handleBreadcrumb(item)">{{ item.name }}</view>
			</block>
		</view>

		<!-- 列表 -->
		<f-list v-for="(item, index) in list" :key="index" :item="item" :index="index" :showRight="false" @click="handleClick(item)"></f-list>
